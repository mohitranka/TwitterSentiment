...
class NBTrainSentimentClassifier(NBSentimentClassifier):
    ...
    def load_train_data(self):
        self.classifier = NaiveBayesClassifier.train(self.all_posfeats[:int(0.60*len(self.all_posfeats))]+\
                                                         self.all_negfeats[:int(0.60*len(self.all_negfeats))])
    ...
    def load_test_data(self):
        self.posfeats = self.all_posfeats[int(0.60*len(self.all_posfeats)):]
        self.negfeats = self.all_negfeats[int(0.60*len(self.all_negfeats)):]
